...
export default function SplashCursor({
  ...
  RAINBOW_MODE = true,
  COLOR = '#d8d83f',
  colors = ['#d8d83f', '#ffffff', '#888888', '#3275F8', '#444444']
}) {
  ...
  useEffect(() => {
    ...
    // Fast exit if WebGL is disabled or reduced motion is preferred
    ...
    if (!webglCtx) return; // Fallback if WebGL isn't supported
    
    const { gl, ext } = webglCtx;

    // Reduce dye resolution on mobile for performance
    const isMobile = window.innerWidth < 768;
    if (!ext.supportLinearFiltering || isMobile) {
    ...
    // Set up site palette interpolation
    const sitePalette = colors.map(hexToRGB);
    ...
    function updateColors(dt: number) {
      // Slower phase shift for elegant color transitions
      colorPhase += dt * config.COLOR_UPDATE_SPEED * 0.1;
      if (colorPhase >= sitePalette.length) {
        colorPhase = 0;
      }

      const idx1 = Math.floor(colorPhase);
      const idx2 = (idx1 + 1) % sitePalette.length;
      const t = colorPhase - idx1;

      const c1 = sitePalette[idx1];
      const c2 = sitePalette[idx2];
    ...
    function hexToRGB(hex: string) {
      ...
      // Multiply by 0.15 for subtle blend intensity
    ...
    function scaleByPixelRatio(input: number) {
      ...
      return Math.floor(input * Math.min(pixelRatio, 2));
    }
